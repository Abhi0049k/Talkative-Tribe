...
import { MessageI, useHomeRight } from "@/hooks/useHomeRight";
...
const HomeRight: FC<HomeRightProps> = ({ socket }) => {
    ...
    return (
        <div className="w-[75%] max-h-full rounded-md mx-1 border flex gap-2 flex-col">
            {
                                ...
                                (
                                    <div className="h-[90vh] flex gap-4 flex-col p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200" ref={chatContainerRef}>
                                        {messages.map((el: MessageI) => {
                                            return (
                                                <Chat key={el.id} msg={el.message} yours={el.senderId === userId} datetime={moment(el.createdAt).format("Do MMM, YYYY, LT")} />
                                            )
                                        })}
                                    </div>
                                )
