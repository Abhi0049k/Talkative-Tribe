...
const HomeLeft: FC<HomeChildProps> = ({ socket }) => {
    const { name, searchList, userId, allPrevPrivateRooms, handleChangeName, handleUserClick, handleChatWithAI, handleDeleteRoom } = useHomeLeft(socket);
    ...
    return (
        <div className={`relative flex flex-col w-full sm:w-[300px] md:w-[320px] h-full border-foreground ${isLeftHanded ? 'sm:border-l-[3px] border-r-0' : 'sm:border-r-[3px]'}`}>
            ...
            {/* Chat List Header with Settings Button */}
            <div className={`px-4 py-3 bg-[hsl(var(--secondary))] border-b-[3px] border-foreground flex items-center justify-between ${isLeftHanded ? 'flex-row-reverse' : 'flex-row'}`}>
                <h2 className="font-bold text-black uppercase tracking-wide text-sm">Conversations</h2>
                <button
                    onClick={() => setIsSettingsOpen(true)}
                    className="p-1.5 hidden sm:block hover:bg-black/10 rounded-sm transition-colors border-2 border-transparent hover:border-black/20"
                    title="Settings"
                >
